## Data with outliers in Experiment 3
setwd("E:/code_mass")
library(tictoc)
library(MASS)
library(class)
...
for (kkk in 1:times) {
  ...
  tt1 = px0 * c2
  tt2 = px0 * c4
  t1 = runif(n = tt1, min = -1, max = 1)
  t2 = runif(n = tt2, min = -1, max = 1)
  x[(1):(n1), 1:px0] = mvrnorm(n = c1, mu1, sigma2)
  x[(n1 + 1):(n2), 1:px0] = matrix(t1, nrow = c2, ncol = px0)
  x[(n2 + 1):(n3), 1:px0] = mvrnorm(n = c3, mu1, sigma2)
  x[(n3 + 1):(n4), 1:px0] = matrix(t2, nrow = c4, ncol = px0)
  
  ### Outliers
  ac1 = floor(0.05 * n1)
  ac2 = floor(0.05 * (n2 - n1))
  ac3 = floor(0.05 * (n3 - n2))
  ac4 = floor(0.05 * (n4 - n3))
  ...
  ac0 = floor(0.05 * px)
  aa00 = sample(1:px, ac0, replace = FALSE)
  x[aa1, aa00] = x[aa1, aa00] + 5
  aa00 = sample(1:px, ac0, replace = FALSE)
  x[aa2, aa00] = x[aa2, aa00] + 5
  aa00 = sample(1:px, ac0, replace = FALSE)
  x[aa3, aa00] = x[aa3, aa00] + 5
  aa00 = sample(1:px, ac0, replace = FALSE)
  x[aa4, aa00] = x[aa4, aa00] + 5
